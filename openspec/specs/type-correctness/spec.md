## Domain: type-correctness (New Capability)

### Purpose
Capture the static type-correctness guarantees and the behavioral contracts that depend on them for the exam image upload endpoint and for safe form-field extraction in page handlers. The domain exists so that future changes cannot silently regress the mypy quality gate or reintroduce the latent runtime bugs surfaced during the baseline fix.
